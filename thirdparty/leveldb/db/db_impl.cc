  ...
  for (; input->Valid() && !shutting_down_.Acquire_Load(); ) {
    ...
    if (!ParseInternalKey(key, &ikey)) {
      ...
      if (last_sequence_for_key <= compact->smallest_snapshot) {
        ...
        drop = true;
      } else if (nexus_gc_key_ > 0) {
        if (ikey.user_key.size() == sizeof(int64_t)) {
          int64_t num = 0;
          memcpy(&num, ikey.user_key.data(), sizeof(int64_t));
          if (num < nexus_gc_key_) {
            drop = true;
            //Log(options_.info_log, "drop %ld < %ld", num, nexus_gc_key_);
          }
        }
      }
